...
export class RegisterRequestDto {
  @ApiProperty({ example: 'user@demo.com' }) @IsEmail() email!: string;
  @ApiProperty({ minLength: 6 }) @IsString() @MinLength(6) password!: string;
  @ApiProperty({ example: 'org-1' }) @IsString() orgId!: string;
  ...
  @ApiPropertyOptional() @IsOptional() @IsString() staffFullName?: string;
